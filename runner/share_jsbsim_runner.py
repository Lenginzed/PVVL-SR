...
class ShareJSBSimRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                done_count = (self.buffer.masks == False).sum()
                train_infos["episodes_in_buffer"] = int(done_count)
                if done_count > 0:
                    train_infos["average_episode_rewards"] = self.buffer.rewards.sum() / done_count
                else:
                ...
                self._write_train_metrics(episode, train_infos)
    ...
    def save(self, episode):
        ...
        torch.save(policy_actor_state_dict, str(self.save_dir) + '/actor_{}.pt'.format(episode))
        policy_critic_state_dict = self.policy.critic.state_dict()
        torch.save(policy_critic_state_dict, str(self.save_dir) + '/critic_latest.pt')
        torch.save(policy_critic_state_dict, str(self.save_dir) + '/critic_{}.pt'.format(episode))
    ...
    def _write_train_metrics(self, episode, train_infos):
        payload = {
            "update": int(episode),
            "total_num_steps": int(self.total_num_steps),
        }
        payload.update({key: _jsonable(value) for key, value in train_infos.items()})
        path = os.path.join(str(self.run_dir), "train_metrics.jsonl")
